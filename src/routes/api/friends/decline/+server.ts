 ...
 * Response:
 *   200: { ok: true }
 *   400: Not a pending request / validation failure
 *   401: Unauthorised
 *   404: Friendship not found
 *
 * @module routes/api/friends/decline
 */
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	// Only parties to the friendship can decline
	if (row.userA !== userId && row.userB !== userId) {
		error(400, 'You are not part of this friend request');
	}

	// Only the recipient can decline (the initiator can just cancel instead)
	if (row.initiatedBy === userId) {
		error(400, 'You cannot decline your own sent request');
	}
