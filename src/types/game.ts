 ...
 *
 * introChunks (Task 2.3.4):
 * Populated by lessonAssembler with one entry per chunk.
 * Used by LessonIntroCard to preview all phrases before the lesson begins.
 * This is the only way LessonView knows what to show in the intro — the
 * assembler is the single source of truth, so we carry the data on the plan.
 * 
...
export interface LessonPlan {
  ...
  /**
   * Preview data for the LessonIntroCard (Task 2.3.4).
   * One entry per chunk, in lesson order.
   * Present only when the assembler has chunk-level data to expose.
   * Absent for legacy/fallback lessons that didn't go through lessonAssembler.
   */
  introChunks?: Array<{
