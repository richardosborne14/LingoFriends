...
This is a **major UX overhaul** that touches most of the app. The current flow (login → chat with AI → occasional activities) becomes (login → garden world → pick a tree/path → structured lessons with concrete activities).

### What stays
- Authentication (Pocketbase, `useAuth`)
- Pocketbase backend + all existing services
- AI service (Groq Llama 3.3) — prompt changes needed
- Voice services (TTS/STT) — used within lessons
- React + TypeScript + Vite + Tailwind foundation
- Existing UI component library (`components/ui/`)

### What changes fundamentally
- Navigation: Garden → Path → Lesson (replaces sidebar + chat)
...
export interface ActivityConfig {
  ...
  sentence?: string;        // For fill_blank — use ___ for the blank
  hint?: string;
...
export interface PlayerAvatar {
  id: string;
  emoji: string;  // Placeholder — will become sprite reference
  name: string;
}

// Modify existing UserProfile — add fields:
// avatar_id: string;
// totalSunDrops: number;
...
/**
 * Sun Drop Service
 * Manages the Sun Drop currency — earning, spending, penalties.
...
src/
├── components/
│   ├── garden/                    # NEW — Garden world
│   │   ├── GardenWorld.tsx        # PixiJS canvas + avatar movement
│   │   ├── GardenTree.tsx         # Tree sprite with health states
│   │   ├── GardenAvatar.tsx       # Player avatar sprite
│   │   ├── GardenDecoration.tsx   # Placeable decorations
│   │   ├── InteractionPanel.tsx   # Popup when near a tree
│   │   └── MobileDpad.tsx         # Touch controls
│   │
│   ├── path/                      # NEW — Skill path view
│   │   ├── PathView.tsx           # Winding trail layout
│   │   ├── LessonNode.tsx         # Individual node on path
│   │   └── PathHeader.tsx         # Skill name + progress
│   │
│   ├── lesson/                    # NEW — Lesson gameplay
│   │   ├── LessonView.tsx         # Main lesson container
│   │   ├── TutorBubble.tsx        # Professor Finch message
│   │   ├── HelpPanel.tsx          # Help chat panel
│   │   ├── LessonComplete.tsx     # End-of-lesson screen
│   │   ├── SunDropBurst.tsx       # Reward animation
│   │   ├── PenaltyBurst.tsx       # Wrong answer animation
...
│   │       ├── MatchingPairs.tsx
│   │       ├── Translate.tsx
│   │       └── ActivityRouter.tsx # Routes type → component
│   │
│   ├── social/                    # NEW — Gifts & friends
│   │   ├── GiftUnlock.tsx         # Gift earned animation
│   │   ├── SendGift.tsx           # Pick friend to send to
│   │   └── FriendGifts.tsx        # Received gifts panel
│   │
│   ├── shared/                    # NEW — Shared game UI
│   │   ├── SunDropIcon.tsx        # Custom SVG icon (normal/broken/glow)
│   │   ├── SunDropCounter.tsx     # Header counter
│   │   ├── MiniTree.tsx           # Small tree for path/UI
│   │   ├── HealthBar.tsx          # Tree health bar
│   │   ├── AvatarPicker.tsx       # Choose animal avatar
│   │   └── AvatarBubble.tsx       # Avatar display circle
│   │
│   ├── ui/                        # KEEP — existing UI components
│   │   └── ... (Button, Card, etc.)
│   │
│   ├── ActivityWidget.tsx         # DEPRECATE → replaced by lesson/activities/
│   ├── ChatInterface.tsx          # DEPRECATE → replaced by LessonView
│   ├── AuthScreen.tsx             # KEEP
│   └── ProfileSettings.tsx        # MODIFY — add avatar, Sun Drops display
│
├── hooks/
│   ├── useAuth.tsx                # KEEP
│   ├── useSessions.tsx            # MODIFY — adapt for lesson-based sessions
│   ├── useGarden.tsx              # NEW — garden state, tree CRUD
│   ├── useSunDrops.tsx            # NEW — Sun Drop earning/spending
│   ├── useLesson.tsx              # NEW — lesson state machine
│   └── useMessageHandler.ts       # DEPRECATE or heavily modify
│
├── services/
│   ├── pocketbaseService.ts       # MODIFY — add tree/gift/garden methods
│   ├── groqService.ts             # MODIFY — new structured output format
│   ├── sunDropService.ts          # NEW (see Step 3)
│   └── lessonGenerator.ts         # NEW — AI lesson plan generation
│
├── App.tsx                        # MAJOR REWRITE — garden-first navigation
└── types.ts                       # MODIFY (see Step 1)
```
...
```
┌──────────────────────────┐
│        Header Bar        │  ← Avatar, Sun Drops, streak
├──────────────────────────┤
│                          │
│      Active View         │  ← Garden, Path, or Lesson
│   (fullscreen content)   │
│                          │
├──────────────────────────┤
│  🌳 Garden │ 📖 Path │👥│  ← Tab bar (hidden during lessons)
└──────────────────────────┘
```

Navigation state machine:
```
Garden (home) ─── tap tree ───→ Path (sub-menu) ─── tap lesson ───→ Lesson (gameplay)
    ↑                                ↑                                    │
    └──── back ────────────────── back ←──────────── complete/back ───────┘
```
...
Rules:
- Vary activity types — never more than 2 of the same type in a row
- tutorText is BRIEF (max 2 sentences) — it frames the activity, not a standalone conversation
...
**A4.** Build `LessonView.tsx` — the lesson gameplay container
...
**A5.** Build `PathView.tsx` — winding top-to-bottom trail
...
**A6.** Build basic `GardenWorld.tsx` — start with CSS/HTML version
  - Grid of tree plots
...
**C1.** Gift earn flow (lesson complete → gift unlock)
**C2.** Gift send flow (pick friend → send → Pocketbase)
**C3.** Gift receive flow (notification → apply to tree)
**C4.** Decoration system (earn items → place in garden)
**C5.** Seed earning mechanics (path completion → new seed)
...
- [ ] Correct answer first try → full Sun Drops + burst
- [ ] Wrong answer → penalty burst + tree shake + -1
- [ ] Retry after wrong → correct gives half
- [ ] Help button → shows hint → next correct gives half
- [ ] Multiple wrongs → cumulative -1 each, floor of 0
- [ ] Can't go below 0 Sun Drops total in a lesson

### Lesson flow testing
- [ ] All activities complete → lesson complete screen
...
1. **Read `GAME_DESIGN.md` before starting** — it's the source of truth for all game mechanics
2. **Use the prototype as UX reference** — `docs/prototypes/prototype-v4-final.jsx` shows exact behavior
3. **Sun Drops, not XP** — replace ALL references to XP throughout the codebase
4. **Garden is home** — there is no other home page or dashboard
5. **Every AI response must include an activity** — no standalone conversational messages during lessons
6. **Wrong answers = danger, not punishment** — dramatic feedback but always allow retry
7. **Trees decay** — this is the spaced repetition mechanic, it's non-negotiable
8. **Follow `.clinerules`** — especially confidence scoring, ask human when uncertain
9. **Phase A first** — get core mechanics working before social features
10. **Test with actual kids** — if it's not fun for a 10-year-old, it's not done
