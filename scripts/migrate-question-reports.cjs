 ...
 * 
 * @run node scripts/migrate-question-reports.cjs
 */
...
const questionReportsSchema = {
  ...
  createRule: '@request.auth.id != "" || @request.auth.id = ""', // Anyone logged in or anonymous
  updateRule: null, // Only admins
  deleteRule: null, // Only admins
  indexes: [
    'CREATE INDEX idx_user ON user',
    'CREATE INDEX idx_activity_type ON activity_type',
    'CREATE INDEX idx_issue_type ON issue_type',
    'CREATE INDEX idx_status ON status',
    'CREATE INDEX idx_created ON created',
  ],
};
